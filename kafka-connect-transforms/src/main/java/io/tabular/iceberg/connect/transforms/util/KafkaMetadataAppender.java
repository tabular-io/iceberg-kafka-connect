...
public class KafkaMetadataAppender {
    ...
    public KafkaMetadataAppender(ExternalKafkaData externalData, Schema schema, String metadataFieldName) {
    ...
    public static class ExternalStringKafkaData implements ExternalKafkaData{
        ...
        public static ExternalKafkaData parse(String field) {
            ...
            if (parts.length != 2) {
                throw new ConfigException(String.format("Could not parse %s for %s", field, KafkaMetadataAppender.EXTERNAL_KAFKA_METADATA));
            }
    ...
    public static KafkaMetadataAppender from(SimpleConfig config) {
        ExternalKafkaData externalAppender = ExternalStringKafkaData.parse(config.getString(EXTERNAL_KAFKA_METADATA));
        String metadataFieldName = config.getString(KEY_METADATA_FIELD_NAME);

        SchemaBuilder schema = SchemaBuilder.struct();

        externalAppender.addToSchema(schema)
    ...
    public SchemaBuilder appendSchema(SchemaBuilder schema) {
        return schema.field(this.metadataFieldName, this.schema);
    }
    ...
    public Struct appendToStruct(SinkRecord record, Struct struct) {
        ...
        if (Objects.nonNull(record.timestamp())) {
