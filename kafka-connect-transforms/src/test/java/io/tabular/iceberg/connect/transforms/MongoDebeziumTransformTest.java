...
public class MongoDebeziumTransformTest {
    ...
    private MongoDebeziumTransform<SinkRecord> getTransformer(String mode) {
        MongoDebeziumTransform<SinkRecord> t = new MongoDebeziumTransform<>();
        t.configure(Collections.singletonMap(MongoDebeziumTransform.ARRAY_HANDLING_MODE_KEY, mode));
        return t;
    }
    ...
    private static Schema getUpdateDescriptionSchema() {
        Schema truncatedArraySchema = SchemaBuilder.struct()
        ...
        return SchemaBuilder.struct()
                ...
                .field("truncatedArrays",
                        SchemaBuilder.array(truncatedArraySchema).optional().build())
                .build();
    }

    private static Struct getUpdateDescriptionStruct(Optional<ArrayList<String>>removedFields, String updatedFields) {
    ...
    private static Schema getMongoValueSchema(boolean validEnvelope) {
        ...
        builder.name((validEnvelope) ? "dbserver1.inventory.customers.Envelope" : "some_invalid_envelope_name");
    ...
    private static <T> T extractFieldAtImpl(Object struct, String path, Function<Object, T> f, Boolean safe) throws Exception {
        try {
            ...
            for (int i = 0; i < size; i++) {
                String p = paths[i];
                if (i == size - 1) {
                    return f.apply(((Struct) target).get(p));
                }
                target = Requirements.requireStruct(target, "extractFieldAt dive").get(p);
            }
    ...
    private static <T> T extractFieldAt(Object struct, String path, Function<Object, T> f) throws Exception {
       return  extractFieldAtImpl(struct, path, f, false);
    }
    ...
    private static Schema.Type extractSchemaTypeAt(Object struct, String path) throws Exception {
        Struct s = Requirements.requireStruct(struct, "extractSchemaAt in test");
        String[] paths = path.split("\\.");
        int size = paths.length;

        Schema target = s.schema();
        for (int i = 0; i < size; i++) {
            String p = paths[i];
            if (i == size - 1) {
                return target.field(p).schema().type();
            }
            target = target.field(p).schema();
        }
    ...
    public void shouldMaintainKey() throws Exception {
        ...
        SinkRecord record = new SinkRecord(TEST_TOPIC, TEST_PARTITION, DEFAULT_KEY_SCHEMA, DEFAULT_KEY_STRUCT, DEFAULT_VALUE_SCHEMA, valueStruct, 0L, DEFAULT_TS_MS, TimestampType.CREATE_TIME);
    ...
    public void shouldConvertCreateEvents() throws Exception {
        ...
        assertThat(extractStructAt(result.value(), "source")).isEqualTo(extractStructAt(valueStruct, "source"));
    ...
    @Test
    @DisplayName("partial updates with before values should merge updatedFields with before values for SinkRecord after struct")
    public void shouldConvertPartialUpdateWithBeforeValues() throws Exception {
        Struct valueStruct = new Struct(DEFAULT_VALUE_SCHEMA);
        valueStruct.put("before", getFile("mongo_update_event_before.json"));
        valueStruct.put("updateDescription", getUpdateDescriptionStruct(Optional.empty(), getFile("mongo_update_event_updated_fields.json")));
        valueStruct.put("source", DEFAULT_SOURCE_STRUCT);
        valueStruct.put("ts_ms", DEFAULT_TS_MS);
        valueStruct.put("op", "u");
        SinkRecord record = new SinkRecord(TEST_TOPIC, TEST_PARTITION, DEFAULT_KEY_SCHEMA, DEFAULT_KEY_STRUCT, DEFAULT_VALUE_SCHEMA, valueStruct, 0L, DEFAULT_TS_MS, TimestampType.CREATE_TIME);
    ...
    public void shouldConvertPartialUpdateWithBeforeValuesRemovedFields() throws Exception {
        ...
        Struct valueStruct = new Struct(DEFAULT_VALUE_SCHEMA);
        valueStruct.put("before", getFile("mongo_update_event_before.json"));
        valueStruct.put("updateDescription", getUpdateDescriptionStruct(Optional.of(removedFields), getFile("mongo_update_event_updated_fields.json")));
        valueStruct.put("source", DEFAULT_SOURCE_STRUCT);
        valueStruct.put("ts_ms", DEFAULT_TS_MS);
        valueStruct.put("op", "u");
        SinkRecord record = new SinkRecord(TEST_TOPIC, TEST_PARTITION, DEFAULT_KEY_SCHEMA, DEFAULT_KEY_STRUCT, DEFAULT_VALUE_SCHEMA, valueStruct, 0L, DEFAULT_TS_MS, TimestampType.CREATE_TIME);
    ...
    @Test
    @DisplayName("partial updates without before/after values should include updateFields and id column for SinkRecord after struct")
    public void shouldConvertPartiaulUpdateBeforeAfterMissing() throws Exception {
        ...
        assertThat(result.valueSchema().field("after").schema().field("_id").schema().type()).isEqualTo(Schema.Type.INT32);
