...
public class KafkaMetadataAppenderTest {
    ...
    private static final Schema SCHEMA_WITH_EXT_FIELD = baseBuilder().field("external", Schema.STRING_SCHEMA);

    private static final ExternalStringKafkaData EXT_FIELD = new ExternalStringKafkaData("external", "value");

    private static final Schema RECORD_SCHEMA = SchemaBuilder.struct().field("id", Schema.STRING_SCHEMA).build();

    private static SinkRecord genSinkRecord(Boolean includeTimestamp) {
        Struct value = new Struct(RECORD_SCHEMA);
        value.put("id", "id");
        if (includeTimestamp) {
            return new SinkRecord(TOPIC, PARTITION, null, null, RECORD_SCHEMA, value, OFFSET, TIMESTAMP, TimestampType.CREATE_TIME);
        } else {
    ...
    private static SinkRecord genSinkRecordAsMap(Boolean includeTimestamp) {
        ...
        if (includeTimestamp) {
            return new SinkRecord(TOPIC, PARTITION, null, null, null, value, OFFSET, TIMESTAMP, TimestampType.CREATE_TIME);
        } else {
    ...
    public void appendSchema() {
        KafkaMetadataAppender appender = new KafkaMetadataAppender(new EmptyExternalData(), SCHEMA, KafkaMetadataAppender.DEFAULT_METADATA_FIELD_NAME);
        KafkaMetadataAppender appenderWithExternalField = new KafkaMetadataAppender(EXT_FIELD, SCHEMA_WITH_EXT_FIELD, KafkaMetadataAppender.DEFAULT_METADATA_FIELD_NAME);
        ...
        assertThat(appendResult.field(KafkaMetadataAppender.DEFAULT_METADATA_FIELD_NAME).schema()).isEqualTo(SCHEMA);
        assertThat(appendExternalResult.field(KafkaMetadataAppender.DEFAULT_METADATA_FIELD_NAME).schema()).isEqualTo(SCHEMA_WITH_EXT_FIELD);
    }
    ...
    public void appendToStruct() {
        ...
        Schema schema = appenderWithExternalField.appendSchema(SchemaBuilder.struct().field("id", Schema.STRING_SCHEMA));

        Struct resultParent = appenderWithExternalField.appendToStruct(recordWithTimestamp, new Struct(schema));
        Struct result = (Struct) resultParent.get(KafkaMetadataAppender.DEFAULT_METADATA_FIELD_NAME);
        Struct resultWithoutTime = (Struct) appenderWithExternalField.appendToStruct(recordWithoutTimestamp, new Struct(schema)).get(KafkaMetadataAppender.DEFAULT_METADATA_FIELD_NAME);
    ...
    public void appendToMap() {
        ...
        Map<String, Object> record1 = Maps.newHashMap("id", "id");
        Map<String, Object> record2 = Maps.newHashMap("id", "id");

        Map<String, Object> expected = ImmutableMap.of(
                "id", "id",
                KafkaMetadataAppender.DEFAULT_METADATA_FIELD_NAME, ImmutableMap.of(
                        "topic", TOPIC,
                        "partition", PARTITION,
                        "offset", OFFSET,
                        "timestamp", TIMESTAMP,
        ...
        Map<String, Object> expectedWithoutTime = ImmutableMap.of(
                "id", "id",
                KafkaMetadataAppender.DEFAULT_METADATA_FIELD_NAME, ImmutableMap.of(
                        "topic", TOPIC,
                        "partition", PARTITION,
        ...
        Map<String, Object> result = appenderWithExternalField.appendToMap(recordWithTimestamp, record1);
        Map<String, Object> resultWithoutTime = appenderWithExternalField.appendToMap(recordWithoutTimestamp,record2);
